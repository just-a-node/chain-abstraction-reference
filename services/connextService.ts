...
export default class ConnextService {
  ...
  async getPoolFeeForUniV3(domainID: string, token0: string, token1: string, rpcUrl: string) {
    try {
  ...
  async getSupportedAssetsForDomain(domainId: number) {
    return getSupportedAssetsForDomain(domainId);
  }
